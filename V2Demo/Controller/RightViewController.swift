...
class RightViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    fileprivate lazy var tableView: UITableView = {
        ...
        tableView .register(RightNodeTableViewCell.self, forCellReuseIdentifier: "\(RightNodeTableViewCell.self)")
    ...
    func maximumRightDrawerWidth() -> CGFloat {
        ...
        for node in rightNodes {
            let size = node.nodeName!.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT),
                                                                height: CGFloat(MAXFLOAT)),
                                                   options: NSStringDrawingOptions.usesLineFragmentOrigin,
                                                   attributes: [NSAttributedString.Key(rawValue: "NSFontAttributeName"):cellFont!], context: nil)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "\(RightNodeTableViewCell.self)", for: indexPath) as! RightNodeTableViewCell
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        V2Client.shareInstance.centerViewController?.tab = node.nodeTab
//        V2Client.shareInstance.centerViewController?.refre
        V2Client.shareInstance.drawerController?.closeDrawer(animated: true, completion: nil)
    }
